...
namespace CortanaExtendedCommands
{
    /// <summary>
    ...
    sealed partial class App : Application
    {
        ...
        protected async override void OnLaunched(LaunchActivatedEventArgs e)
        {
            ...
            if (e.PrelaunchActivated == false)
            {
                ...
                try
                {
                    StorageFile vcd = await Package.Current.InstalledLocation.GetFileAsync(@"CortanaExtendedCommands.xml");
        ...
        protected async override void OnActivated(IActivatedEventArgs args)
        {
            ...
            {
                VoiceCommandActivatedEventArgs cmd = (VoiceCommandActivatedEventArgs)args;
                SpeechRecognitionResult res = cmd.Result;
                string commandname = res.RulePath[0];
                MessageDialog dialog = new MessageDialog("");
                //main switch of cortana custom commands
                switch (commandname)
                {
                    case "SpegniComputer":
                        dialog.Content = "Spegnimento del computer";
                        break;
